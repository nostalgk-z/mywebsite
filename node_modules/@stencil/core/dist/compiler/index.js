    ...
    return __awaiter$6(this, void 0, void 0, function* () {
        const promises = Object.keys(results).map((moduleType) => {
            ...
            const promises = Object.keys(jsModuleList)
                ...
                .map((chunk) => __awaiter$6(this, void 0, void 0, function* () {
...
function copyTasks(config, compilerCtx, diagnostics, commit) {
    return __awaiter$8(this, void 0, void 0, function* () {
        ...
        try {
            ...
            yield Promise.all(copyTasks.map((copyTask) => __awaiter$8(this, void 0, void 0, function* () {
                yield processCopyTasks(config, compilerCtx, allCopyTasks, copyTask);
            })));
            yield Promise.all(allCopyTasks.map((copyTask) => __awaiter$8(this, void 0, void 0, function* () {
...
function writeAppManifest(config, compilerCtx, buildCtx) {
    return __awaiter$9(this, void 0, void 0, function* () {
        ...
        if (config.generateDistribution) {
            ...
            yield compilerCtx.fs.writeFile(manifestFilePath, JSON.stringify(manifestData, null, 2));
        }
        return manifestData;
    });
}
function serializeAppManifest(config, manifestDir, entryModules, globalModule) {
...
function parseDependentManifest(config, collectionName, includeBundledOnly, manifestDir, manifestJson) {
...
function parseComponents(config, includeBundledOnly, manifestDir, manifestData, manifest) {
...
function parseStyle(config, manifestDir, cmpData, modeStyleData) {
    ...
    if (modeStyleData.stylePaths) {
        modeStyle.externalStyles = modeStyleData.stylePaths.map(stylePath => {
            const externalStyle = {};
            externalStyle.absolutePath = normalizePath(config.sys.path.join(manifestDir, stylePath));
...
function serializeAppGlobal(config, manifestDir, manifestData, globalModule) {
    if (!globalModule) {
        return;
    }
...
function getCollectionDirDestination(config, assetsMeta) {
    ...
    return pathJoin(config, config.collectionDir, config.sys.path.relative(config.srcDir, assetsMeta.absolutePath));
}
...
function readPackageJson(config, compilerCtx, buildCtx) {
    return __awaiter$12(this, void 0, void 0, function* () {
        const packageJsonPath = config.sys.path.join(config.rootDir, 'package.json');
        let packageJsonText;
        try {
            packageJsonText = yield compilerCtx.fs.readFile(packageJsonPath);
        }
        catch (e) {
...
function validatePackageJsonTypes(config, diagnostics, data) {
...
function generateTypes(config, ctx) {
    return __awaiter$12(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const indexDtsSrcContent = yield ctx.fs.readFile(indexDtsSrcFilePath);
            yield ctx.fs.writeFile(indexDtsDistTypesFilePath, indexDtsSrcContent);
        }
        catch (e) { }
        // copy the generated components.d.ts file
        try {
            const componentsDtsSrcContent = yield ctx.fs.readFile(getComponentsDtsSrcFilePath(config));
            yield ctx.fs.writeFile(getComponentsDtsDistTypesFilePath(config), componentsDtsSrcContent);
        }
        catch (e) { }
    });
}
...
function getComponentsDtsDistTypesFilePath(config) {
...
function writeBuildFiles(config, compilerCtx, buildCtx) {
    return __awaiter$14(this, void 0, void 0, function* () {
        ...
        yield writeAppManifest(config, compilerCtx, buildCtx);
...
function setArrayConfig(config, configName) {
    const userConfigName = getUserConfigName(config, configName);
    if (typeof config[userConfigName] === 'function') {
        config[userConfigName] = config[userConfigName]();
    }
    ...
    return {
        transform: function (sourceText, id, context) {
            return __awaiter$16(this, void 0, void 0, function* () {
                ...
                const cachedContent = yield context.cache.get(cacheKey);
                if (cachedContent != null) {
                    ...
                    if (typeof minifyResults.output === 'string') {
                        ...
                        yield context.cache.put(cacheKey, results.code);
                    }
                }
                return results;
            });
    ...
    return {
        transform: function (sourceText, id, context) {
            return __awaiter$17(this, void 0, void 0, function* () {
                ...
                results.code = yield new Promise(resolve => {
                    nodeSass.render(renderOpts, (err, sassResult) => __awaiter$17(this, void 0, void 0, function* () {
                        if (err) {
                            ...
                            yield context.fs.writeFile(results.id, css, { inMemoryOnly: true });
                            resolve(css);
                        }
                    }));
                });
...
function loadDiagnostic(context, sassError, filePath) {
    ...
    if (filePath) {
        ...
        d.relFilePath = formatFileName$1(context.config.rootDir, d.absFilePath);
        d.header = formatHeader$1('sass', d.absFilePath, context.config.rootDir, sassError.line);
        if (sassError.line > -1) {
            try {
...
function formatFileName$1(rootDir, fileName) {
    if (!rootDir || !fileName)
        return '';
    fileName = fileName.replace(rootDir, '');
    if (/\/|\\/.test(fileName.charAt(0))) {
...
function formatHeader$1(type, fileName, rootDir, startLineNumber = null, endLineNumber = null) {
    let header = `${type}: ${formatFileName$1(rootDir, fileName)}`;
    if (startLineNumber !== null && startLineNumber > 0) {
        if (endLineNumber !== null && endLineNumber > startLineNumber) {
            header += `, lines: ${startLineNumber} - ${endLineNumber}`;
        }
...
function rebuild(config, ctx, watcher) {
...
function configFileReload(config) {
...
class WatcherListener {
    ...
    queueCopyTasks() {
        ...
        this.copyTaskTmr = setTimeout(() => __awaiter$18(this, void 0, void 0, function* () {
...
function initWatcher(config, compilerCtx) {
    ...
    if (compilerCtx.hasSuccessfulBuild || !config.watch)
        return;
...
function getBuildContext(config, compilerCtx, watcher) {
    ...
    buildCtx.finish = () => __awaiter$19(this, void 0, void 0, function* () {
        try {
...
class InMemoryFileSystem {
    ...
    copyDir(src, dest, opts) {
        return __awaiter$21(this, void 0, void 0, function* () {
            ...
            yield Promise.all(dirItems.map((dirItem) => __awaiter$21(this, void 0, void 0, function* () {
    ...
    removeDir(dirPath) {
        return __awaiter$21(this, void 0, void 0, function* () {
            ...
            try {
                ...
                yield Promise.all(dirItems.map((item) => __awaiter$21(this, void 0, void 0, function* () {
    ...
    commitWriteFiles(filesToWrite) {
        return Promise.all(filesToWrite.map((filePath) => __awaiter$21(this, void 0, void 0, function* () {
            if (typeof filePath !== 'string') {
    ...
    commitDeleteFiles(filesToDelete) {
        return Promise.all(filesToDelete.map((filePath) => __awaiter$21(this, void 0, void 0, function* () {
            if (typeof filePath !== 'string') {
...
function loadTypeScriptDiagnostics(rootDir, resultsDiagnostics, tsDiagnostics) {
    ...
    for (var i = 0; i < maxErrors; i++) {
        resultsDiagnostics.push(loadDiagnostic$1(rootDir, tsDiagnostics[i]));
    }
}
function loadDiagnostic$1(rootDir, tsDiagnostic) {
    const d = {
        level: 'error',
        type: 'typescript',
        language: 'typescript',
...
function generateStyles(config, compilerCtx, buildCtx, entryModules) {
    return __awaiter$35(this, void 0, void 0, function* () {
        yield Promise.all(entryModules.map((bundle) => __awaiter$35(this, void 0, void 0, function* () {
            yield Promise.all(bundle.moduleFiles.map((moduleFile) => __awaiter$35(this, void 0, void 0, function* () {
...
function generateComponentStyles(config, compilerCtx, buildCtx, moduleFile) {
    return __awaiter$35(this, void 0, void 0, function* () {
        ...
        yield Promise.all(Object.keys(stylesMeta).map((modeName) => __awaiter$35(this, void 0, void 0, function* () {
            // compile each style mode's sass/css
            const styles = yield compileStyles(config, compilerCtx, buildCtx, moduleFile.jsFilePath, stylesMeta[modeName]);
            // format and set the styles for use later
            yield setStyleText(buildCtx, moduleFile.cmpMeta, stylesMeta[modeName], styles);
        })));
    });
}
function compileStyles(config, compilerCtx, buildCtx, jsFilePath, styleMeta) {
    return __awaiter$35(this, void 0, void 0, function* () {
        const absStylePath = styleMeta.externalStyles.map(externalStyle => {
        ...
        const styles = yield Promise.all(absStylePath.map((filePath) => __awaiter$35(this, void 0, void 0, function* () {
            filePath = normalizePath(filePath);
            const transformResults = yield runPluginTransforms(config, compilerCtx, buildCtx, filePath);
            return transformResults.code;
        })));
...
function setStyleText(buildCtx, cmpMeta, styleMeta, styles) {
...
function getEntryEncapsulations(entryModule) {
    ...
    entryModule.moduleFiles.forEach(m => {
        const encapsulation = m.cmpMeta.encapsulation || 0;
...
function injectRegisterServiceWorker(config, compilerCtx, swConfig, indexHtml) {
    return __awaiter$36(this, void 0, void 0, function* () {
        ...
        if (match) {
            ...
            if (config.minifyJs) {
                const minifyResults = yield minifyJs(config, compilerCtx, serviceWorker, 'es5', false);
                minifyResults.diagnostics.forEach(d => {
                    config.logger[d.level](d.messageText);
                });
...
function setIndexHtmlContent(config, compilerCtx, indexHtml) {
    return __awaiter$37(this, void 0, void 0, function* () {
        ...
        if (!swConfig && config.devMode) {
            ...
            indexHtml = yield injectRegisterServiceWorker(config, compilerCtx, swConfig, indexHtml);
        }
...
function genereateReadme(config, ctx, moduleFile, dirPath) {
    return __awaiter$38(this, void 0, void 0, function* () {
        ...
        try {
            const content = yield ctx.fs.readFile(readMePath);
            // update
...
function upgradeCollection(config, compilerCtx, buildCtx, manifest) {
    return __awaiter$41(this, void 0, void 0, function* () {
        try {
            ...
            yield doUpgrade(manifest, upgradeTransforms);
...
function createDoUpgrade(config, compilerCtx, buildCtx) {
    return (manifest, upgrades) => __awaiter$41(this, void 0, void 0, function* () {
        ...
        yield Promise.all(manifest.moduleFiles.map((moduleFile) => __awaiter$41(this, void 0, void 0, function* () {
            try {
...
function loadConfigCollection(config, compilerCtx, buildCtx, configCollection) {
    return __awaiter$42(this, void 0, void 0, function* () {
        ...
        const packageJsonStr = yield compilerCtx.fs.readFile(dependentPackageJsonFilePath);
        ...
        const dependentManifestJson = yield compilerCtx.fs.readFile(dependentManifestFilePath);
        ...
        collectionManifest = parseDependentManifest(config, configCollection.name, configCollection.includeBundledOnly, dependentManifestDir, dependentManifestJson);
        ...
        yield upgradeCollection(config, compilerCtx, buildCtx, collectionManifest);
        if (config.generateDistribution) {
            yield copySourceCollectionComponentsToDistribution(config, compilerCtx, collectionManifest.moduleFiles);
        }
...
function copySourceCollectionComponentsToDistribution(config, compilerCtx, modulesFiles) {
    return __awaiter$42(this, void 0, void 0, function* () {
        ...
        collectionModules.forEach(m => {
            copyTasks.push({
                src: m.jsFilePath,
                dest: config.sys.path.join(config.collectionDir, COLLECTION_DEPENDENCIES_DIR, m.originalCollectionComponentPath)
            });
                    ...
                    if (styleMeta.externalStyles) {
                        styleMeta.externalStyles.forEach(externalStyle => {
                            copyTasks.push({
                                ...
                                dest: config.sys.path.join(config.collectionDir, COLLECTION_DEPENDENCIES_DIR, externalStyle.originalCollectionPath)
                            });
                        });
                    }
                });
        ...
        yield Promise.all(copyTasks.map((copyTask) => __awaiter$42(this, void 0, void 0, function* () {
            yield compilerCtx.fs.copy(copyTask.src, copyTask.dest);
        })));
    });
}
...
function createRendererPatch(plt, domApi) {
    ...
    function patchVNode(oldVNode, newVNode) {
        ...
        if ('svg' === newVNode.vtag && isSvgMode)
            isSvgMode = false;
    }
...
function findExternalLoaderScript(doc, loaderExternalSrcUrl) {
...
function getFilePathFromUrl(config, fromUrl, toUrl) {
...
function optimizeHtml(config, ctx, doc, styles, opts, results) {
    return __awaiter$46(this, void 0, void 0, function* () {
        ...
        if (opts.inlineLoaderScript !== false) {
            ...
            try {
                yield inlineLoaderScript(config, ctx, doc, results);
            }
            catch (e) {
                results.diagnostics.push({
                    level: 'error',
        ...
        if (opts.inlineAssetsMaxSize > 0) {
            try {
                yield inlineExternalAssets(config, ctx, results, doc);
            }
            catch (e) {
                results.diagnostics.push({
                    level: 'error',
...
function hydrateHtml(config, ctx, cmpRegistry, opts) {
    return new Promise(resolve => {
        ...
        plt.onAppLoad = (rootElm, styles, failureDiagnostic) => __awaiter$47(this, void 0, void 0, function* () {
            if (config._isTesting) {
...
function gatherMetadata(config, typechecker, sourceFileList) {
...
function visitFactory(config, checker, componentMetaList, diagnostics) {
 ...
 */
function generateComponentTypesFile(config, cmpList) {
    let typeImportData = {};
    const allTypes = {};
...
function scanDirForTsFiles(config, fs, dir) {
    return __awaiter$52(this, void 0, void 0, function* () {
        ...
        yield Promise.all(tsFileItems.map((tsFileItem) => __awaiter$52(this, void 0, void 0, function* () {
...
function build(config, compilerCtx, watcher) {
    return __awaiter$53(this, void 0, void 0, function* () {
        ...
        try {
            ...
            yield loadCollections(config, compilerCtx, buildCtx);
            if (buildCtx.shouldAbort())
                return buildCtx.finish();
